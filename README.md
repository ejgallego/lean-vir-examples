...
This repository is a minimal client of the `lean_vir` Lake package.
...
`scripts/fetch-sdk.sh` reads the pinned `lean_vir` revision from
`lake-manifest.json` and asks `lean_vir/vir_fetch_sdk` for the matching GitHub
Actions artifact. The SDK install fails if the downloaded artifact was built
from a different commit. GitHub requires authentication for Actions artifact
downloads, so set `GITHUB_TOKEN` or run `gh auth login` once before using this
path locally.
...
The example imports the Lake-facing module path:
